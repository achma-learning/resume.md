---
type: topo
titre: "Hépatite C — Virus de l'Hépatite C (VHC)"
date: "2026-04-14"
module: "[[Hépato-Gastroentérologie]]"
matiere: "Hépatologie — Infectiologie"
enseignant: "Pr. "
semestre: 
annee: 2025-2026
audio: "[[]]"
...
```mermaid
graph TD
    A[Contamination VHC] --> B[Incubation: 2 sem – 6 mois<br/>Moy: 7 semaines]
    B --> C[Hépatite aiguë C<br/>Asymptomatique: ~80%]
    C --> D{Évolution à 6 mois}
    D --> |20-30%| E[Résolution spontanée<br/>Ac anti-VHC+ / ARN–]
    D --> |70-80%| F[Hépatite CHRONIQUE C<br/>ARN+ > 6 mois]
    F --> G{Progression fibreuse<br/>20-30 ans}
    G --> |20-30%| H[CIRRHOSE]
    G --> |70-80%| I[Fibrose légère à modérée<br/>peu évolutive]
    H --> J[Décompensation<br/>hépatique]
    H --> K[CHC<br/>1-4%/an sur cirrhose]
```
...
```mermaid
flowchart TD
    A[Suspicion / Dépistage VHC] --> B[Sérologie VHC<br/>Ac anti-VHC ELISA 3e génération<br/>Ou TROD VHC]
    B --> C{Résultat ?}
    C --> |Négatif| D[Pas d'infection VHC<br/>Sauf infection récente avant séroconversion<br/>ou immunodépression sévère]
    C --> |Positif| E[Charge virale VHC<br/>PCR ARN quantitative RT-PCR]
    E --> F{ARN détectable ?}
    F --> |Non| G[Infection ancienne guérie<br/>ou guérison spontanée<br/>Ac anti-VHC persistants à vie]
    F --> |Oui| H[INFECTION ACTIVE<br/>Bilan pré-thérapeutique]
    D --> |Suspicion forte + facteurs risque| I[PCR ARN VHC d'emblée<br/>si infection récente probable]
```
...
```mermaid
graph LR
    T0[Début AAD<br/>J0] --> T8[Fin AAD<br/>8 ou 12 semaines]
    T8 --> T20[12 sem après fin<br/>ARN VHC ???]
    T20 --> |ARN indétectable| RVS[RVS12 = GUÉRISON<br/>~97-99% des cas]
    T20 --> |ARN détectable| ECHEC[Échec virologique<br/>→ Spécialiste + Vosevi®]
    RVS --> SUIVI[Suivi post-guérison]
```
